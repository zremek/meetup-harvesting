...
whyR_meetup <- function(group_url = "https://www.meetup.com/Spotkania-Entuzjastow-R-Warsaw-R-Users-Group-Meetup"){
  ...
  what_can_be_clicked <-
    unlist(
      lapply(remDr$findElements("class name", "eventCard--link"), function(driverElement){
  ...
  while ("More Meetups" %in% what_can_be_clicked) {
    remDr$findElements("class name", "eventCard--link")[[which("More Meetups" == what_can_be_clicked)]]$clickElement()
    Sys.sleep(3)
    what_can_be_clicked <-
    unlist(
      lapply(remDr$findElements("class name", "eventCard--link"), function(driverElement){
